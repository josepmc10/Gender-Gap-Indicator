...
ACM$ACMD = 1 - (dnorm(ACM$ACM/100,0.5,0.09) / dnorm(0.5,0.5,0.09))
...
```{r, message=FALSE}
df <- full_join(full_join(full_join(E2E,ACM), full_join(PAR,GEI)), full_join(SAL, VVG))

GGI <- function(df, year){
  df = df[df$Year==year,]
  
  ECO <- df$SALD * 0.3 + df$PARD * 0.3 + df$ACMD * 0.3
  SOC <- df$RVVG * 0.3 + df$GEI * 0.3
  CUL <- df$DE2ED
  
  return(ECO*0.3 + SOC*0.3 + CUL*0.3)
}
